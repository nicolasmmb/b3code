...
async def test_at_lists_file_created_on_disk(tmp_path: Path):
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.pause()
        labels = [item.label for item in screen.query_one(Autocomplete).suggestions]
        assert any("late.py" in label for label in labels)
