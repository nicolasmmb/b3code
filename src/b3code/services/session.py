...
class SessionStore:
    ...
    def messages(self) -> list[ModelMessage]:
        if not self._current.messages:
            return []
        return ModelMessagesTypeAdapter.validate_python(self._current.messages)
    ...
    def _save(self) -> None:
        ...
        self.path.write_text(self._file.model_dump_json(indent=2) + "\n", encoding="utf-8")
        ...
        if isinstance(msg, ModelRequest):
            ...
            for part in msg.parts:
                if isinstance(part, ToolCallPart):
                    args = part.args_as_json_str() if hasattr(part, "args_as_json_str") else str(part.args)
                    turns.append(
                        DisplayTurn(
                            role="tool",
                            text=args,
